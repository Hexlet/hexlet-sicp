<?php

return [
    'title' => 'Stream of random numbers',
    'description' => [
        '1' =>
        "Exercise ",
        '2' =>
        " discussed generalizing the random-number generator to allow one to reset the random-number sequence so as to produce repeatable sequences of ''random'' numbers. " .
        "Produce a stream formulation of this same generator that operates on an input stream of requests to ",
        '3' =>
        " a new random number or to ",
        '4' =>
        " the sequence to a specified value and that produces the desired stream of random numbers. " .
        "Don't use assignment in your solution.",
    ],
];
