...
<header class="navbar navbar-expand-lg">
  <nav class="container py-2 border-bottom">
    ...
    <div class="navbar-collapse collapse" id="navbar-responsive">
      <ul class="navbar-nav">
        <li class="nav-item"><a href="{{ route('chapters.index') }}" class="nav-link p-2">{{ __('layout.nav.chapters')
            }}</a></li>
        <li class="nav-item"><a href="{{ route('exercises.index') }}" class="nav-link p-2">{{ __('layout.nav.exercises')
            }}</a></li>
        <!-- TODO: translate guide to english -->
        @if (App::isLocale('ru'))
        <li class="nav-item"><a href="https://guides.hexlet.io/how-to-learn-sicp/" class="nav-link p-2">{{
            __('layout.nav.sicp_read') }}</a></li>
        @endif
      ...
      <ul class="navbar-nav ms-md-auto">
        ...
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle px-2 link-secondary" id="dropdownFlagButton" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" href="#">
            <img src="{{ LocalizationHelper::getPathToLocaleFlag($currentLocale) }}" alt="{{ LocalizationHelper::getNativeLanguageName($currentLocale) }}" class="me-1" width="24">
          ...
          <ul class="dropdown-menu dropdown-menu-right x-min-w-0" aria-labelledby="dropdownFlagButton">
            @foreach(LocalizationHelper::getOtherLocales($currentLocale, $locales) as $localeCode => ['native' =>
            $language])
            <li>
              <a href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}" rel="alternate" class="dropdown-item" hreflang="{{ $localeCode }}">
                <img src="{{ LocalizationHelper::getPathToLocaleFlag($localeCode) }}" alt="{{ LocalizationHelper::normalizeNativeLanguageName($language) }}" class="mr-1" width="24">
